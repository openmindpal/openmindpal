...
export async function listDeviceEvidenceArtifacts(params: { pool: Pool; tenantId: string; spaceId: string; deviceId?: string | null; limit?: number }) {
  const limit = Math.max(1, Math.min(200, params.limit ?? 50));
  if (params.deviceId) {
    const res = await params.pool.query(
      `
        SELECT *
        FROM artifacts
        WHERE tenant_id = $1 AND space_id = $2 AND type = 'device_evidence' AND (source->>'deviceId') = $3
        ORDER BY created_at DESC
        LIMIT $4
      `,
      [params.tenantId, params.spaceId, params.deviceId, limit],
    );
    return res.rows.map(toArtifact);
  }
  const res = await params.pool.query(
    `
      SELECT *
      FROM artifacts
      WHERE tenant_id = $1 AND space_id = $2 AND type = 'device_evidence'
      ORDER BY created_at DESC
      LIMIT $3
    `,
