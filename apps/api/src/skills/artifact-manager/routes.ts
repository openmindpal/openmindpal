...
import { createArtifact, getArtifact, getArtifactContent, listArtifactsByType, listDeviceEvidenceArtifacts } from "./modules/artifactRepo";
import { consumeArtifactDownloadTokenByHash, createArtifactDownloadToken, generateDownloadToken, hashDownloadToken } from "./modules/artifactDownloadTokenRepo";
...
export const artifactRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/artifacts/device-evidence", async (req) => {
    setAuditContext(req, { resourceType: "artifact", action: "read" });
    const decision = await requirePermission({ req, resourceType: "artifact", action: "read" });
    req.ctx.audit!.policyDecision = decision;
    const subject = req.ctx.subject!;
    if (!subject.spaceId) throw Errors.badRequest("缺少 spaceId");
    const q = z
      ...
      .parse(req.query);
    const items = await listDeviceEvidenceArtifacts({ pool: app.db, tenantId: subject.tenantId, spaceId: subject.spaceId, deviceId: q.deviceId ?? null, limit: q.limit ?? 50 });
    req.ctx.audit!.outputDigest = { type: "device_evidence", count: items.length, deviceId: q.deviceId ?? null };
    return { items };
  });
