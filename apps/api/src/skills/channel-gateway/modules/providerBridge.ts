...
import { extractTextForPromptInjectionScan, getPromptInjectionDenyTargetsFromEnv, getPromptInjectionModeFromEnv, scanPromptInjection, shouldDenyPromptInjectionForTarget, summarizePromptInjection } from "../../safety-policy/modules/promptInjectionGuard";
...
export async function handleBridgeEvents(ctx: { app: any; req: any; reply: any }, opts?: { expectedProvider?: string }) {
  ...
  try {
    ...
    if (cfg.deliveryMode === "async") {
      ...
      const resp = { correlation: { requestId: ctx.req.ctx.requestId, traceId: ctx.req.ctx.traceId }, status: "processing" as const };
      await finalizeIngressEvent({ pool: ctx.app.db, id: inserted.id, status: "queued", responseStatusCode: 202, responseJson: resp });
      ctx.req.ctx.audit!.outputDigest = { status: "queued", provider: body.provider, workspaceId: body.workspaceId, eventId: body.eventId, outboxId: queued.id };
      ctx.reply.status(202);
      return resp;
    }
