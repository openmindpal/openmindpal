...
import { getDevicePolicy } from "../modules/devices/devicePolicyRepo";
import { getDeviceRecord } from "../modules/devices/deviceRepo";
import { cancelDeviceExecution, claimDeviceExecution, completeDeviceExecution, createDeviceExecution, getDeviceExecution, listDeviceExecutions, listPendingByDevice } from "../modules/devices/deviceExecutionRepo";
...
export const deviceExecutionRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/device-agent/executions/:deviceExecutionId/result", async (req) => {
    ...
    req.ctx.audit!.outputDigest = { deviceExecutionId: done.deviceExecutionId, status: done.status, outputDigest: digestObject(body.outputDigest ?? null), evidenceCount: body.evidenceRefs?.length ?? 0, errorCategory: body.errorCategory ?? null };
