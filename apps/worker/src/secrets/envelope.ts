...
export async function decryptSecretPayload(params: {
  ...
  if (params.encFormat !== "envelope.v1") return decryptJson(params.masterKey, params.encryptedPayload as EncryptedPayload);
...
export async function encryptSecretEnvelopeWithKeyVersion(params: {
  pool: Pool;
  tenantId: string;
  masterKey: string;
  scopeType: string;
  ...
  const env: EnvelopeV1 = { format: "envelope.v1", keyRef: { scopeType: params.scopeType, scopeId: params.scopeId, keyVersion: params.keyVersion }, payload: payloadEnc, wrappedDek };
  return env;
}
