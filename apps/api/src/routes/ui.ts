...
import { getLatestReleasedToolVersion, getToolVersionByRef } from "../modules/tools/toolRepo";
...
import { pageDraftSchema, pageViewPrefsSchema } from "../modules/uiConfig/pageModel";
import { validateUiAgainstRegistry } from "../modules/uiConfig/componentRegistry";
import { getDraft, getLatestReleased, listPages, publishFromDraft, rollbackToPreviousReleased, upsertDraft } from "../modules/uiConfig/pageRepo";
...
export const uiRoutes: FastifyPluginAsync = async (app) => {
      ...
      if (k === "new") {
        drafts.push({
          ...
          draft: {
            ...
            actionBindings: [{ action: "create", toolRef: createTool.toolRef }],
          },
        });
      }
      if (k === "edit") {
        drafts.push({
          ...
          draft: {
            ...
            actionBindings: [{ action: "update", toolRef: updateTool.toolRef }],
          },
        });
      }
    }
  ...
  app.post("/ui/pages/:name/publish", async (req, reply) => {
    ...
    if (!released) throw Errors.uiConfigDenied("发布失败");
